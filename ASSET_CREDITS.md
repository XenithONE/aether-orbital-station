...
- The version 2.1 command instruments, split ceramic chassis, curved screens, antireflective surfaces, composite floor, route graphics and HELIOS flight computer are original procedural geometry and Canvas/SVG artwork. They are implemented in `src/command-console.ts`, `src/materials.ts`, `src/interior.ts`, and `src/warp-ui.ts`.
- The version 2.1 curved warp passage, converging particles, travelling rings and expanding exit aperture are original Three.js/TSL geometry and shaders in `src/warp-effects.ts`; they use no additional downloaded images.
